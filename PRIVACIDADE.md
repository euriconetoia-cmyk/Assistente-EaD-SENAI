...
A versão 3.7.10 permite escolher uma pasta do computador pelo seletor nativo do Chrome. A extensão recebe acesso somente à pasta escolhida e apenas depois da autorização do usuário. A referência da pasta pode ser mantida no IndexedDB da extensão para facilitar exportações posteriores. O Chrome pode solicitar nova autorização e o usuário pode negar ou revogar o acesso.
